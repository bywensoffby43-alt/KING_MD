...
import { setCode } from './lib/pair.js';
store.readFromFile();
...
const phoneNumber = config.pairingNumber || config.ownerNumber || "50940127120";
...
global.botname = config.botName || "KIND_MD";
global.themeemoji = "•";
    ...
    try {
        ...
        if (pairingCode && !isRegistered) {
            ...
            if (config.pairingNumber) {
                ...
                phoneNumberInput = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFormat: 50940127120 (without + or spaces) : `)));
            }
            else {
        ...
        QasimDev.ev.on('connection.update', async (s) => {
            ...
            if (connection === "open") {
                ...
                printLog('info', `[ ${config.botName || 'KING_MD'} ]`);
...
async function main() {
    ...
    printLog('info', 'Starting KIND_MD...');
